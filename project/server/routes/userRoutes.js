...
router.post("/register", async (req, res) => {
  ...
  try {
    ...
    const userExists = await User.findOne({ userEmail: req.body.userEmail }); // Ensure this matches the schema field name
    ...
    const hashPwd = await bcrypt.hash(req.body.userPassword, salt);
    ...
    res.send({
      success: true,
      message: "Mubarak Hoo, Aapka account ban gaya hai!. Abb aap login kar sakte hain!",
    });
  } catch (err) {
    console.error(err);
    res.status(500).send({
